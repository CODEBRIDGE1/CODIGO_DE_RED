...
def upgrade() -> None:
    # ### SAFE MIGRATION: Add columns as nullable first, then update, then set NOT NULL ###
    
    # Step 1: Add all columns as NULLABLE
    op.add_column('companies', sa.Column('razon_social', sa.String(length=300), nullable=True))
    ...
    op.add_column('companies', sa.Column('rpu', sa.String(length=50), nullable=True))
    op.add_column('companies', sa.Column('tipo_suministro', sa.String(length=50), nullable=True))
    ...
    op.add_column('companies', sa.Column('is_active', sa.Boolean(), nullable=True, server_default='true'))
    
    # Step 2: Update existing rows with defaults (safe for existing data)
    op.execute("UPDATE companies SET razon_social = COALESCE(name, 'Sin nombre') WHERE razon_social IS NULL")
    op.execute("UPDATE companies SET rpu = CONCAT('RPU-', id::text) WHERE rpu IS NULL")
    op.execute("UPDATE companies SET tipo_suministro = 'DESCONOCIDO' WHERE tipo_suministro IS NULL")
    op.execute("UPDATE companies SET is_active = true WHERE is_active IS NULL")
    
    # Step 3: Now make them NOT NULL (safe because data is populated)
    op.alter_column('companies', 'razon_social', nullable=False)
    op.alter_column('companies', 'rpu', nullable=False)
    op.alter_column('companies', 'tipo_suministro', nullable=False)
    op.alter_column('companies', 'is_active', nullable=False)
    
    # Alter RFC with explicit PostgreSQL USING for safe casting
    op.alter_column('companies', 'rfc',
               existing_type=sa.VARCHAR(length=20),
               type_=sa.String(length=13),
               nullable=False,
               postgresql_using='SUBSTRING(rfc, 1, 13)')
