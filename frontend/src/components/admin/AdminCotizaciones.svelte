<script lang="ts">
  ...
  function printQuotePDF() {
    ...
    const ivaLabel = q.iva_percent > 0 ? `I.V.A. (${q.iva_percent}%)` : 'I.V.A.';
    ...
    // Construir dirección de la organización emisora
    let orgDireccion = '';
    if (org) {
      const partes = [
    ...
    if (q.rfc) clienteLineas.push(`RFC: <strong>${q.rfc}</strong>`);
    if (q.direccion) {
      let dir = q.direccion;
      if (q.ciudad) dir += ', ' + q.ciudad;
      if (q.estado_empresa) dir += ', ' + q.estado_empresa;
      if (q.codigo_postal) dir += ' C.P. ' + q.codigo_postal;
      clienteLineas.push(dir);
    }
...
<body>
  ...
  <div class="doc-header">
    <div class="header-left">
      ...
      <div class="org-detail">
        ...
        ${org?.ciudad ? `${org.ciudad}, ` : ''}${org?.estado || ''}${org?.codigo_postal ? ` C.P. ${org.codigo_postal}` : ''}<br/>
  ...
  <table class="meta-table">
    ...
    <tr>
      ...
      <td class="meta-val"><strong>${q.cliente_nombre || '—'}</strong></td>
    </tr>
    <tr>
      <td class="meta-label">Cliente</td>
      <td class="meta-val" colspan="3">
        <strong>${q.razon_social || '—'}</strong>
        ${clienteLineas.length ? '<br/>' + clienteLineas.join('<br/>') : ''}
      </td>
    </tr>
    ${q.observaciones ? `<tr>
